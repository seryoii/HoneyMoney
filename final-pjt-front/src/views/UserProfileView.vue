      ...
      <v-row>
        <v-col cols="6">
          ...
          <v-col class="text-center">
            <v-container>
              <v-card-title class="ibm-plex-sans-kr-bold"
                ><h4>나의 금융 정보</h4></v-card-title
              >
              <div class="px-4 mb-2">
                ...
                >
                <v-chip class="chip-style mx-1 my-1"
                  >나의 저축 성향 : {{ userProfile.tendency }}</v-chip
                >
                <v-chip class="chip-style mx-1 my-1"
                  >나의 저축 희망 기간 :
                  {{ userProfile.desirePeriod }}개월</v-chip
                >
              </div>
            </v-container>
          </v-col>
        ...
        <v-col class="px-0 mx-0" cols="6">
          <v-row class="px-0 mx-0">
            <v-col class="d-flex align-center justify-center px-0 mx-0 mb-0">
              <v-container
                v-if="depositStore.profileDepositData.length"
                class="py-0 mb-0"
              >
                <v-row justify="end">
                ...
                <v-carousel height="300" show-arrows="hover" hide-delimiters>
                  <v-carousel-item
                    v-for="(
                      honeyDeposit, index
                    ) in depositStore.profileDepositData"
                    :key="`carousel1-${index}`"
                    class="no-padding"
                  >
                    <v-card
                      class="mx-16 fill-height d-flex align-center justify-center"
                      elevation="0"
                    >
                      <v-card
                        class="card-design mb-2 density-compact"
                        prepend-avatar="https://randomuser.me/api/portraits/women/10.jpg"
                        variant="text"
                      >
                        <template v-slot:subtitle>
                          <span class="ibm-plex-sans-kr-regular">{{
                            honeyDeposit.kor_co_nm
                          }}</span>
                        </template>
                        <template v-slot:title>
                          <span
                            class="font-weight-black ibm-plex-sans-kr-regular"
                          >
                            <h4>{{ honeyDeposit.fin_prdt_nm }}</h4>
                          </span>
                        </template>
                        <v-card-text class="mt-3 pb-0 ibm-plex-sans-kr-regular"
                          >가입 방법 : {{ honeyDeposit.join_way }}</v-card-text
                        >
                        <v-card-text class="ibm-plex-sans-kr-regular"
                          >만기 후 이자율 :
                          {{ honeyDeposit.mtrt_int }}</v-card-text
                        >
                        <v-container align="center">
                          <v-btn
                            color="yellow-darken-3"
                            variant="tonal"
                            @click="viewDetailDeposit"
                            >Details</v-btn
                          >
                          <v-dialog
                            v-model="viewDetailDepositDialog"
                            width="1000"
                          >
                            <v-card class="mx-auto" width="1000">
                              <template v-slot:subtitle>
                                <v-row>
                                  ...
                                  <v-col align="center" cols="4">
                                    <div
                                      class="custom-subtitle ibm-plex-sans-kr-regular"
                                    >
                                      {{ honeyDeposit.kor_co_nm }}
                                    </div>
                                  </v-col>
                                  <v-col
                                    align="center"
                                    cols="4"
                                    class="mt-2 ibm-plex-sans-kr-regular"
                                    >꿀바르기</v-col
                                  >
                                </v-row>
                              </template>
                              <template v-slot:title>
                                <v-row align="center" justify="space-between">
                                  ...
                                  <v-col align="center" class="pb-0 mt-2">
                                    <v-img
                                      v-if="
                                        honeyDeposit.interest_user &&
                                        !honeyDeposit.interest_user.includes(
                                          userStore.userInfo.id
                                        )
                                      "
                                      @click="
                                        saveEvent(
                                          honeyDeposit.fin_prdt_cd,
                                          honeyDeposit.fin_prdt_nm
                                        )
                                      "
                                      :src="cancel"
                              ...
                              <v-row justify="center" align="center">
                                ...
                                <v-col>
                                  <v-card-text class="">{{
                                    honeyDeposit.interest_user?.length
                                  }}</v-card-text>
                                </v-col>
                              </v-row>
                              <hr />
                              ...
                              <v-row justify="center" align="center">
                                ...
                                <v-col>
                                  <v-card-text class="">
                                    {{
                                      honeyDeposit.join_deny == 1
                                        ? "제한없음"
                                        : honeyDeposit.join_deny == 2
                                        ? "서민전용"
                                        : honeyDeposit.join_deny == 3
                                        ? "일부제한"
                                        : "기타"
                                    }}
                                  </v-card-text>
                              ...
                              <v-row justify="center" align="center">
                                ...
                                <v-col>
                                  <v-card-text class="">{{
                                    honeyDeposit.etc_note
                                  }}</v-card-text>
                                </v-col>
                              </v-row>
                              <v-card-actions>
                                <v-btn @click="viewDetailDepositDialog = false"
                                  >OK</v-btn
                                >
                              </v-card-actions>
                            </v-card>
              ...
              <v-container v-else>
                <v-container
                  class="my-14 py-14 font-weight-black ibm-plex-sans-kr-regular"
                  align="center"
                  justify="center"
                >
                  <p>예금 꿀통이 비어있어요...</p>
          ...
          <v-row class="py-0 my-0">
            <v-col class="d-flex align-center justify-center py-0 my-0">
              <v-container
                class="py-0 mb-0"
                v-if="savingStore.profileSavingData.length"
              >
                <v-row justify="end">
                  <v-col class="ms-4" cols="2">
                    <v-badge
                      color="yellow"
                      :content="savingStore.profileSavingData.length"
                    ></v-badge>
                    <v-img :src="jar" max-width="50"></v-img>
                  </v-col>
                  <v-row class="py-0 my-0" align="end">
                ...
                <v-carousel height="300" show-arrows="hover" hide-delimiters>
                  <v-carousel-item
                    v-for="(
                      honeySaving, index
                    ) in savingStore.profileSavingData"
                    :key="`carousel2-${index}`"
                    class="no-padding"
                  >
                    <v-card
                    ...
                    >
                      ...
                      >
                        ...
                        <v-card-text class="mt-3 pb-0 ibm-plex-sans-kr-regular"
                          >가입 방법 : {{ honeySaving.join_way }}</v-card-text
                        >
                        <v-card-text class="ibm-plex-sans-kr-regular"
                          >만기 후 이자율 :
                          {{ honeySaving.mtrt_int }}</v-card-text
                        >
                        <v-container align="center">
                          <v-btn
                            color="yellow-darken-3"
                            variant="tonal"
                            @click="viewDetailSaving"
                            >Details</v-btn
                          >
                          <v-dialog
                            v-model="viewDetailSavingDialog"
                            width="1000"
                          >
                            <v-card class="mx-auto" width="1000">
                              <template v-slot:subtitle>
                              ...
                              <template v-slot:title>
                                <v-row align="center" justify="space-between">
                                  ...
                                  <v-col align="center" class="pb-0 mt-2">
                                    <v-img
                                      v-if="
                                        honeySaving.interest_user &&
                                        !honeySaving.interest_user.includes(
                                          userStore.userInfo.id
                                        )
                                      "
                                      @click="
                                        saveEvent(
                                          honeySaving.fin_prdt_cd,
                                          honeySaving.fin_prdt_nm
                                        )
                                      "
                                      :src="cancel"
                                      class="button-image hover-effect"
                              ...
                              <v-row justify="center" align="center">
                                ...
                                <v-col>
                                  <v-card-text class="">{{
                                    honeySaving.interest_user?.length
                                  }}</v-card-text>
                                </v-col>
                              </v-row>
                              <hr />
                              ...
                              <v-row justify="center" align="center">
                                ...
                                <v-col>
                                  <v-card-text class="">
                                    {{
                                      honeySaving.join_deny == 1
                                        ? "제한없음"
                                        : honeySaving.join_deny == 2
                                        ? "서민전용"
                                        : honeySaving.join_deny == 3
                                        ? "일부제한"
                                        : "기타"
                                    }}
                                  </v-card-text>
                              ...
                              <v-row justify="center" align="center">
                                ...
                                <v-col>
                                  <v-card-text class="">{{
                                    honeySaving.etc_note
                                  }}</v-card-text>
                                </v-col>
                              </v-row>
                              <v-card-actions>
                                <v-btn @click="viewDetailSavingDialog = false"
                                  >OK</v-btn
                                >
                              </v-card-actions>
                            </v-card>
